"""定期读取飞书多维表格中的打印机，并回填打印机状态。

表格字段：
  ip_address  打印机局域网 IP
  serial      打印机序列号
  access_code 局域网访问码
  status      脚本回填的状态

运行：
  python printer_status_bitable_worker.py
"""
...
def query_printer(ip_address: str, serial: str, access_code: str) -> str:
    ...
    # Bambu 打印机通常使用自签名证书；局域网 MQTT 连接需关闭证书校验。
    ...
    except Exception as exc:  # 网络异常必须回填，不能让整个轮询停止
